...
def generate_data(num_transactions, num_customers):
    customers = []
    transactions = []
    unique_merchants = set()  # Initialize a set to store unique merchant names

    # Generate unique merchant names and add to the set
    while len(unique_merchants) < 10:
        merchant_name = f"Merchant{random.randint(21, 30)}"
        unique_merchants.add(merchant_name)

    external_data = {
        "blacklist_info": list(unique_merchants),  # Convert set to list for JSON serialization
        "credit_scores": {},
        "fraud_reports": {}
    }
    ...
    for i in range(num_transactions):
        ...
        transaction = {
            ...
            "currency": random.choice(["USD", "EUR", "GBP"]),
            "merchant_details": f"Merchant{random.randint(1, 30)}",
            "customer_id": customer_id,
            "transaction_type": random.choice(["purchase", "withdrawal"]),
...
def get_external_data():
    # Return the generated external data
